    ...
    def __call__(self, request):
        if request.user.is_authenticated and request.path != reverse('core:logout'):
            if request.user.role == 'customer':
                ...
                pattern=re.compile(r'^' + reverse('customers:review_form', kwargs={'service_id': 1}).replace('1', r'\d+') + r'$')
                
                if any((payment_pattern.match(request.path) or pattern.match(request.path)) or request.path.startswith(url) for url in allowed_urls):
                    pass
                else:
